...
check_deps() {
    ...
    if command -v python3 &>/dev/null; then
        ...
        echo ""
        echo "  Python is needed for the book seeder tool."
        echo "  (If you don't plan to use the seeder, this is optional.)"
        echo ""
        read -rp "  Install Python now? (Y/n): " choice
        if [[ "$(lower "$choice")" != "n" ]]; then
            install_python
        fi
    fi
...
install_python() {
    ...
    case $pkg in
        ...
        *)      err "Can't auto-install Python. Install manually: https://python.org"; return 1 ;;
    esac
...
check_pip_packages() {
    ...
    python3 -c "import bs4"     2>/dev/null || missing=1

    if [ "$missing" -eq 1 ]; then
        echo -ne "  Installing Python packages (requests, beautifulsoup4)..."
        if python3 -m pip install --quiet --break-system-packages requests beautifulsoup4 2>/dev/null \
            || python3 -m pip install --quiet requests beautifulsoup4 2>/dev/null; then
            echo ""
            ok "Python packages: installed."
        else
            echo ""
            warn "pip install had trouble. Try: pip3 install requests beautifulsoup4"
        fi
    else
    ...
    if ! python3 -c "import anthropic" 2>/dev/null; then
        warn "anthropic SDK not installed (optional — only needed for AI-generated book assessments)"
        echo "    To add it later: pip3 install anthropic"
    fi
}
...
gen_secret() {
    # Try openssl first, fall back to python, fall back to /dev/urandom
    local len="${1:-32}"
...
ensure_env() {
    ...
    local jwt_secret litellm_key pg_pass
    jwt_secret=$(gen_secret 32)
    litellm_key=$(gen_secret 32)
    pg_pass=$(gen_secret 16)

    ok "Internal secrets generated (JWT, LiteLLM, Postgres)"
    echo ""
    echo "  APE needs at least one LLM provider key to work."
    echo "  You can also skip this and add keys later in the web UI."
    echo ""

    local anthropic_key="" openai_key=""
    read -rsp "  Anthropic API key (paste, or Enter to skip): " anthropic_key
    echo ""
    read -rsp "  OpenAI API key (paste, or Enter to skip): " openai_key
    echo ""

    cat > .env <<ENVEOF
# Generated by ape.sh — $(date -u +"%Y-%m-%d %H:%M UTC")
# These are internal secrets. Don't share this file.
...
ENVEOF
    ...
    echo ""
    echo "  After startup, open ${C}http://localhost:8070${N} to finish setup."
    echo ""
}
...
do_start() {
    ...
    if $cmd up -d; then
        ...
        echo ""
        echo "  (You can open those links in your browser)"
    else
        echo ""
...
do_seed() {
    ...
    echo ""
    if ! command -v python3 &>/dev/null; then
        err "Python isn't installed yet."
        echo "  Run this script again and choose to install it when asked."
        return 1
    fi

    echo "  This will populate the database with book theme data."
    echo "  It scrapes triggerwarningdatabase.com (takes ~5 min)"
    echo "  and optionally generates AI assessments (needs API key)."
    echo ""
    read -rp "  Start seeding? (Y/n): " choice
    if [[ "$(lower "$choice")" == "n" ]]; then
        return 0
    fi
    echo ""
    python3 "$SCRIPT_DIR/book_ape_seeder.py" --all --ape-url http://localhost:8069
}
...
show_menu() {
    ...
    echo "  7) Seed books     - Populate book database"
    echo "  8) New            - Create concept-space / collection / domain"
    echo "  9) List           - Show concept-spaces"
    echo "  0) Exit"
    echo ""
}
